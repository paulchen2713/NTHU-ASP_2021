function ans_varying = APA_varying(train, data, L_order, step_size,M)
    % 
    % initialization
    % 
    ...
    for i = 1 : len_data
        ...
        if i < L_order
            ...
            if count <= len_train && mod(i, 450) <= len_train && mod(i, 450) > M + L_order - 2
    ...
    z_ans = output_z(1, :);
    %
    % fetch result for Time-Varing Channel
    %
    result = [];
    for h=0:499
        result = [result z_ans(51+h*450:450+h*450) ];
    end
    ans_varying = result ;
end
